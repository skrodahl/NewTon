...
- **One source for the default bracket view.** The per-format, per-size default zoom + pan values were duplicated in two places (the initial render in `renderBracket()` and the Reset-zoom button in `resetZoom()`), and only one copy carried the "needs tuning" notes — a drift waiting to happen. Extracted `getDefaultView(format, bracketSize)`; both call sites use it. No behavior change (the two copies held identical values).
- **32-player backside background uses the shared builder.** The faint gradient box behind the backside bracket was drawn via a shared `createBacksideBackground()` for 8- and 16-player brackets, but the 32-player renderer inlined its own copy of the same logic. It now calls the shared function (which already had a matching 32-player branch) like the other sizes. No visual change.
- **32-player backside placement labels follow the rendered match positions.** For 8- and 16-player brackets, the elimination-place labels above the backside bracket read the match Y positions from the render pass; the 32-player labels instead recomputed those positions with a parallel copy of the geometry — correct today, but liable to silently drift out of alignment if the 32-player layout ever changed. The 32-player renderer now records each round's first-match Y as it draws it, and the labels read those values — so labels can no longer desync from the matches. No visual change (per-round label positions verified numerically identical).

### Tournament export (Phase 6 cleanup)
...
- **One definition of how player names are matched.** Analytics identifies players by name — each tournament stores its own player ids, so there is nothing stable to join on across tournaments. That means every view has to agree on exactly how a name is tidied up before comparing it, or the same person can appear twice, or a player's record can differ between two screens. That tidying was written out by hand in twenty places. It is now defined once. Alongside it, the Leaderboard and the Players tab had identical copies of the match win/loss count — they now share one, so those two screens cannot drift into reporting different records for the same player. No change to any figure on screen.
...
- **Analytics loads its data in parallel, and stops reloading it when nothing changed.** The Dashboard, Leaderboard, Players and Matches views each walked the finalized tournaments one at a time, waiting for one tournament's matches to come back from the database before asking for the next. They now request them all at once. On top of that, the loaded matches are kept for reuse: flipping the point mode (Original/Current) or toggling the Ranking and Attendance layers only changes how the numbers are *multiplied*, not which matches exist — but every toggle used to re-read everything from the database anyway. Those toggles now do no database reads at all. With a dozen tournaments loaded, rendering the four views drops from 48 reads to 12, and a toggle drops from 48 to none. Same numbers on screen, noticeably less waiting.
- **Analytics notices when you edit a finished tournament.** Analytics keeps the finalized tournaments it has loaded in memory, but nothing told it to let go of them when a match in one of those tournaments was undone or re-entered — something possible through the maintenance workflow for correcting a completed tournament. Analytics could therefore keep showing pre-edit results until the page was reloaded. Completing and undoing a match now clear that cached copy once the change has been written, so returning to Analytics shows the corrected data.

- **Drawing the bracket reads the match history once instead of once per match.** Every match card asks whether its result can still be undone, and that check re-read and re-parsed the tournament's entire transaction log out of localStorage — about 63 full parses per redraw on a 32-player bracket, repeated after every lane assignment, referee change and completed match. The history is now parsed a single time per redraw and shared by all the cards. Same undo indicators, same behavior; the bracket just stops doing the same expensive work sixty-odd times in a row.
...
### Undo internals (Phase 6 cleanup)

- **One source for "what's blocking this undo?".** Two separate places worked out whether a completed match could still be undone: one answering yes/no (which decides whether a winner's name is clickable to undo it), the other producing the wording you see on hover and in Match Controls — *"Cannot Undo, blocked by FS-3-1 (live)"*. Both walked the progression table the same way, and they had to be kept in step by hand: when QR completions were added, the same fix had to be made twice, and it was missed in one of them (fixed in v5.1.5). They now share a single `getUndoBlockingMatches()` that returns the blocking matches; one caller checks whether the list is empty, the other turns it into the sentence. Each keeps its own separate rules — read-only tournaments, walkovers, and matches with no undoable result — since those differ. No behavior change: the two implementations were verified equivalent before the merge, and the merged version was checked against the old one across 1,200 randomized tournament states in both formats at every bracket size (32,400 comparisons, no differences).
...
- `js/newton-history.js` — Phase 6: new `_playerKey(name)` (single definition of the player-name normalization, replacing 20 inline copies) + `_tallyMatchWinLoss(matches, playerMap)` shared by the Leaderboard and Players tab (6.11); `viewBracket()` stashes the prior real `currentTournament` into `_preAnalyticsPreviewTournament` before overwriting (skips re-stashing when the prior is already a preview); Phase 6: new `_matchesByTournament` cache + `_loadMatchesFor()` (`Promise.all`, input order preserved) replacing the four sequential `for … await` match fetches in `renderDashboard`/`renderPlayersTab`/`renderLeaderboard`/`renderAllMatches`, cleared by `_invalidateCache()` (6.5); extracted `_achPoints(stats, p)` helper, replacing 5 duplicated achievement-points formulas; replaced 2 inline base64 visit decodes with `NewtonStats.decodeVisits()`
...
- `js/dynamic-help-system.js` — Phase 6 (6.4): whole-body MutationObserver removed; new `setHelpPage(pageId)` called from `showPage()`
...
- `Docs/CODE-IMPROVEMENT-PLAN.md` — Phase 4 complete (4.3/4.6 done, 4.2 corrected, 4.10 closed); Phase 6 in progress (6.1 part A, 6.2, 6.3, 6.4, 6.5, 6.6, 6.7, 6.8, 6.9, 6.10, 6.11, 6.12–6.20 done; 6.21 closed; 6.1 part B + the renderLeaderboard split declined)
...
### Persistence & data-integrity hardening (Phase 4)

Phase 4's error-state fixes. Deferred to v5.1.6+ by choice: analytics-preview isolation (4.3 — reassessed as a low-priority analytics-only-mode edge), NewtonDB write atomicity (4.6), and the schema-version design item (4.10).

- **A corrupt tournaments registry no longer bricks the app.** New `readTournamentsRegistry()` helper parses `dartsTournaments` inside a try/catch and returns an empty array (with a one-time warning) instead of throwing — one bad value previously broke loading, deletion, and the Recent Tournaments list at once, with `loadRecentTournaments()` failing as a silent unhandled rejection that left the panel blank. Applied to the six read-only sites. The two read-modify-write-back sites (`saveTournamentOnly`, import) kept their plain parses, now covered by the 4.2 save wrap below — a corrupt value there throws and aborts loudly rather than a `[]`-returning reader overwriting the other saved tournaments. `getPlayerList()` is guarded the same way.
...
- **Analytics can't paint stale data on rapid toggles.** Each of the three point-mode-sensitive views (dashboard, leaderboard, players) now carries a per-view generation token: a render captures it at entry and bails before painting if a newer render of the same view has started. Rapidly toggling point mode / layers / scope can no longer let a slower, older render win the repaint.
- **Malformed tournament imports fail cleanly instead of crashing at render.** `validateTournamentData` now rejects the crash-inducing shapes — a non-string player name (crashes `name.toLowerCase()`), a match without a string `id` (crashes `id.startsWith('BS-')`) or with a non-object player slot, or non-numeric `placements` — and coerces `completed` to boolean. Scope deliberately stops at crash-prevention: deeper *semantic* validation (are the match ids real bracket slots?) and an export tamper-CRC were considered and declined, because the data is browser-local with no system of record and no auth/secrets — a tampered import only ever affects the importer's own copy (rationale in the plan's Appendix A.3).
- **Running out of storage no longer loses a match silently.** localStorage has a ~10 MB cap; a long-lived instance that never prunes can fill it, and a `QuotaExceededError` mid-save previously threw into the match-completion flow — the completed match lost on reload, no message. Two contained pieces: a **pre-emptive gate** blocks creating or importing a tournament when storage is ≥90% full, with a clear "free up space" message (and the reassurance that deleting a finalized tournament keeps its stats in Analytics); and a **loud-fail wrap** around `saveTournamentOnly()` that, on a quota error *or* a corrupt registry, alerts once and aborts without a partial/overwriting write instead of throwing. The write order (list first) means the most likely failure leaves both keys untouched and consistent. Deliberately *not* done: the original write-reordering / caller-contract surgery — the gate makes a mid-save quota failure rare enough that the 5-line wrap is a sufficient backstop.

### Interface

- **Storage Space dialog restyled to the new landscape dialog system.** Migrated from the old `.modal-content` layout to the `.dlg dlg--wide` component — metrics in a left sidebar, usage bar + guidance in the main column — matching the Reset/Delete dialogs. Reused the existing dialog classes (no new CSS), and removed ~60 lines of orphaned `storage-*` chrome CSS left behind by the migration.
...
- `js/tournament-management.js` — strict-matchId export pruning; `playerList` restore key; `isOldFormatImport` on overwrite path; watermark walkover filter; debug-log-before-null-check; `showingAllTournaments`/`toggleTournamentView` removed; new `readTournamentsRegistry()` guarded reader (6 read-only sites); Phase 3: escaped tournament status / Recent Tournaments / import messages / watermark, `loadRecentTournaments` delegated listener (server filename + ids out of onclick); Phase 4.4: deeper `validateTournamentData` crash-prevention (string player names, match shape, numeric placements); Phase 4.2: `storageGateBlocks()` gate at create/import + loud-fail try/catch on `saveTournamentOnly`; Storage Space dialog rebuilt as `.dlg--wide`
- `css/styles.css` — removed ~60 lines of orphaned `storage-*` dialog chrome CSS (after the Storage Space restyle)
...
- `js/newton-history.js` — match detail tournament record; import confirm handler cleanup; tiebreak legs excluded from averages; register-tab render error placeholders (`renderTournamentList`/`renderAllMatches`); Phase 3: cell action buttons moved to `data-*` + a shared capture-phase delegated listener (`_wireTableActions`), cell escaping; Phase 4.8: per-view render-race tokens in `renderDashboard`/`renderLeaderboard`/`renderPlayersTab`
...
- `Docs/CODE-IMPROVEMENT-PLAN.md` — new: full review findings and the six-phase plan (Phases 1, 2, 3, 5 and most of Phase 4 shipped — 4.1–4.2, 4.4, 4.5, 4.7–4.9; 4.3/4.6/4.10 deferred; Phase 6 → v5.1.6)
...
- **Pagination buttons** — rows-per-page and page navigation buttons match the Lens preset button style.
...
| `NEWTON_BASE_URL` | URL | — | Base URL for canonical/OG tags. Existing variable. |
...
The QR round-trip closes. The Tournament Manager can now scan a result QR generated by the Chalker. Two entry points:

- **Confirm Match Winner modal** — "Scan Result QR" button sits in the Final Score header row. Camera opens, BarcodeDetector scans, CRC verified, tournament and server IDs checked. On success: decoded result preview with visit scores per leg.
- **Match Controls footer** — "QR Results" button opens the same scanner without a pre-selected match. The match ID is resolved from the payload itself.

Both paths show the same result preview: match ID, format, winner, leg-by-leg visit scores (base64 decoded), and checkout darts per leg.

### QR Payload Inspector in Developer Console

A new **QR Payload Inspector** command scans any NewTon QR — assignment or result — and displays the full raw payload with a pass/fail check for every field: CRC, type, tournament ID, server ID, match found. The current TM's tournament ID and server ID are shown at the top for direct comparison. Never blocks on validation failures — always shows what arrived.

### Files changed

- `tournament.html` — Scan Result QR button in `winnerConfirmModal` Final Score header; `winnerConfirmModal` widened to 800px; QR Results button in Match Controls footer; `qrResultScanModal`; `qrResultPreviewModal`; QR Payload Inspector link in Developer Console Commands
...
- `docker/nginx.conf` — `camera=(self)` for `location ~ \.php$` (enables `getUserMedia` in Docker deployment)
...
The Chalker can now scan the assignment QR generated by the Tournament Manager. Tap QR in the "New Match?" dialog — the camera opens immediately, no extra steps. Scan the code, verify the details, and start.

### QR button in "New Match?" dialog

A new **QR** button sits between "New Match" and "Network" in the Chalker's New Match dialog. It appears whether or not a previous match exists.

### Camera → scan → confirm → start

- Camera opens immediately in a modal using the `BarcodeDetector` API
- Scans automatically — no shutter button
- CRC-32 signature verified on every scan (`newton-integrity.js`)
- On success: camera closes, confirmation screen shows player names, format, lane, and referee
- Operator confirms → starting player selection → match begins
- Lane shown in the match info bar throughout the match

### Error handling

- Wrong QR type (not an assignment payload): error shown in scan modal, camera stays open
- CRC mismatch: error shown, camera stays open — no corrupted data ever accepted
- Camera permission denied: clear message
- Unsupported browser (non-Chrome/Edge): "not supported" message instead of camera

### Files changed

- `js/main.js` — version bump to 5.0.0-beta.2
- `chalker/index.html` — QR button in settings modal; `#qr-scan-modal` and `#qr-confirm-modal` markup; `newton-integrity.js` script tag
- `chalker/js/chalker.js` — `handleQRModeFromModal`, `startQRScanner`, `stopQRScanner`, `handleQRPayload`, `showQRScanError`, `showQRConfirm`, `startMatchFromQR`, `cancelQRScan`, `cancelQRConfirm`; `isModalActive()` updated; event listeners wired
- `chalker/js/newton-integrity.js` — new file, identical copy of TM integrity module
- `chalker/styles/chalker.css` — `.qr-scan-modal-content`, `.qr-video`, `.qr-scan-hint`, `.qr-scan-error`, `.qr-confirm-details`
- `chalker/sw.js` — `newton-integrity.js` added to cache; version bumped to `chalker-v96`
- `Docs/QR.md` — decisions updated: QR and Network are separate buttons; QR workflow documented
