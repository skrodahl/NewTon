...
> **Status (2026-09-06): IN PROGRESS.** Done so far: **6.6**, **6.7**, **6.9**, **6.10**, **6.12**–**6.21** (2026-08-09, browser-verified where visual), **6.3**, **6.1 part A**, **6.8**, **6.5**, **6.4**, **6.2** and **6.11** (2026-09-06). **Phase 6 is complete** apart from two documented declines: **6.1 part B** (`checkRefereeConflict` O(n²) — pure in-memory, unmeasured payoff) and the `renderLeaderboard` split inside 6.11 (see that row). Both are reopenable with evidence. Each item is independent.

### Performance

| # | Where | Issue | Fix |
|---|-------|-------|-----|
| 6.1 ✅ DONE (part A) | `js/bracket-rendering.js` | Every match card re-parses full transaction history from localStorage: `renderMatch()` → `isMatchUndoable()` → `getTournamentHistory()` — ~63 full `JSON.parse` calls per `renderBracket()` on a 32-player DE bracket, and `renderBracket()` runs after every lane/referee/completion change. `checkRefereeConflict()` per card (1423) adds an O(n²) scan | **DONE 2026-09-06, BROWSER-VERIFIED** (undoable matches still undoable; tooltips and the undo action both reference the correct matches) **— per-render memo.** New module-level `_renderPassHistory`, primed in `renderBracket()` immediately before `renderCleanBracket()` and cleared in a `finally`; `isMatchUndoable()` uses `_renderPassHistory \|\| getTournamentHistory()`. N parses per render → 1. **Why a memo, not a `history` argument:** `isMatchUndoable()` has exactly one caller (`renderMatch()`), but `renderMatch()` has ~50 call sites across the 8/16/32 DE and SE renderers — threading a parameter would be a large mechanical edit for the same result. **Why not cache inside `getTournamentHistory()`:** the history key has 8 writers (undo, prune, import, delete, …) that would all need to invalidate. The pass-scoped memo can't go stale — the pass is synchronous, covers SE (the dispatch is inside `renderCleanBracket`), and nothing reachable from it calls `saveTransaction()`; on re-entry the fallback is the old behavior (correct, just slower). Verified with a node/`vm` harness loading the real files against an 8P DE fixture: per-card verdicts byte-identical cached vs. uncached (incl. the AUTO gate, live-downstream and MANUAL-downstream blocks, and one `true`), localStorage reads 9 → 0 (+1 prime), and a sabotage run confirms the check is discriminating. **Part B (the `checkRefereeConflict` O(n²)) deliberately NOT done:** it is pure in-memory work (~63×63 iterations, `getMatchState`/`canMatchStart` touch nothing but the in-memory match objects) — cheap next to what part A removes, and caching it would add a second per-pass structure for an unmeasured payoff. Reopen only if profiling says otherwise. |
| 6.2 ✅ DONE | `js/clean-match-progression.js` | One `completeMatch()` runs `saveTournament()`/`updateResultsTable()` 2–4 times (full JSON serialization each) | **DONE 2026-09-06, BROWSER-VERIFIED** (full DE tournament played through; BS-FINAL 3rd place recorded). Five `saveTournament()` and four `updateResultsTable()` calls on the success path collapsed to **one of each**, placed immediately before `processAutoAdvancements()` (preserving the save-before-auto-advance ordering; walkovers re-enter this same path and persist their own results). Safe because every reader in between works off the in-memory object — verified, not assumed: `updateResultsTable()` documents *"use global tournament object, not localStorage"*, and `autoUploadTournament()` → `buildTournamentPayload()` also builds from memory (localStorage only for the transaction history, already written by `saveTransaction()`), so deferring the save cannot upload stale data. The success path has **no early return** (the only `return`s are inside promise callbacks), so the single save is always reached. **One structural fix required for equivalence:** the four lines computing `isTournamentFinal` sat *outside* any try — a throw there would have skipped a trailing save, where the old code had already persisted. Moved inside the try that immediately follows (they are used only there). **The plan's "related" note was wrong:** `showMatchCommandCenter()` was not double-scheduled — the 500 ms branch is gated on `window.commandCenterWasOpen`, a flag **never set to `true` anywhere in the repo**, only read and cleared in three places. So it was dead code, not a timing bug; the reopen branch and both vestigial clear-the-flag blocks are deleted, and no reference to the flag remains. Net −17 lines. **Verified by differential test** against the pre-change file from git: full tournaments played to completion in both (DE + SE at 8/16/32 **and 13 players**, the last exercising byes/walkovers and `processAutoAdvancements`), alternating MANUAL/QR completions with achievements on half the matches. The complete state — placements, status/readOnly, every match's completed/active/winner/loser, all player stats, the persisted `currentTournament` payload, and the transaction history — is compared **after every single completion**, not just at the end: identical in all 8 cases, 199 completions total. Saves drop ~2× (32-player DE: **127 → 63**; results refreshes 125 → 62). Harness required a seeded `Math.random` (bracket seeding is random — otherwise the two versions play different tournaments) and a frozen clock; a mutant run (BS-FINAL 3rd place not set) fails exactly the 4 DE cases and no SE case. |
| 6.3 ✅ DONE | `js/tournament-management.js` | `updateTournamentWatermark` re-reads and re-parses the full `currentTournament` blob from localStorage on every save, when the global `tournament` object is authoritative | **DONE 2026-09-06.** Deleted the `getItem` + `JSON.parse` + local shadowing `let tournament`; the body now resolves to the global. Verified authoritative: all four writers of the localStorage record (`saveTournamentOnly`, the load path, analytics `viewBracket`, the preview-stash restore) serialize *from* the global, so the blob can never be fresher — and the sibling `updateTournamentStatus()` already read the global. The one state where they diverge (`exitAnalyticsBracketPreview` nulls the global but restores the stashed record) never reaches the watermark: it's analytics-only mode, where the panel is hidden, and no caller runs on that path. Added JSDoc. Net −12/+11 lines. |
| 6.4 ✅ DONE | `js/dynamic-help-system.js`, `js/main.js` | MutationObserver on the whole body subtree incl. attributes — fires on every DOM mutation in the app (bracket renders, clock ticks), each running `querySelector('.page.active')` | **DONE 2026-09-06.** Observer deleted; new `setHelpPage(pageId)` in dynamic-help-system.js (sets `helpState.currentPage`, then `updateHelpContext()`, which is already a no-op while the help modal is closed), called from `showPage()` in main.js. **Wiring choice:** hooked from `showPage()` rather than from `onPageChange()` — the two are equivalent in reach (`onPageChange` has exactly one caller, `showPage`), but `onPageChange` lives in `clean-match-progression.js` and this way the foundation file stays untouched. Safe because `showPage()` is the **only** mutator of `.page.active` repo-wide (main.js:457/463; the sole other occurrence is the static `class="page active"` on `#setup` in tournament.html). **Fixes a latent bug:** `showPage(savedPage)` restores the last page at `DOMContentLoaded`, but the observer was only installed 500 ms later (`tournament.html:37`), so it never saw that transition — reloading onto e.g. the Tournament page left `helpState.currentPage` at `'setup'`, and F1 showed Setup help. The synchronous hook catches it. Verified by a contract test loading the real file: default unchanged, navigation tracked, no repaint while help is closed, repaint when open, same-page and `undefined` are no-ops, and the load-restore transition now tracked. **No MutationObserver remains anywhere in the codebase.** |
| 6.5 ✅ DONE | `js/newton-history.js`, `js/clean-match-progression.js`, `js/bracket-rendering.js` | Four views fetch per-tournament matches *sequentially* (`for … await`), and every point-mode/layer toggle refetches everything from IndexedDB though only client-side multipliers changed | **DONE 2026-09-06.** Three parts. (1) New `_loadMatchesFor(tournaments)` uses `Promise.all`, so the four sites (`renderDashboard`, `renderPlayersTab`, `renderLeaderboard`, `renderAllMatches`) issue their reads concurrently; input order is preserved, which `renderAllMatches` depends on. (2) New `_matchesByTournament` Map cached alongside `_allTournaments` and cleared by `_invalidateCache()` — toggles now do **zero** DB reads. (3) `_invalidateCache()` was **not** wired to the two paths that mutate match records (`NewtonDB.saveMatch` in `completeMatch`, `NewtonDB.deleteMatch` on undo), so a match cache would have gone stale in the `?tm` unlock-and-edit workflow. Added the guarded call at both, hung off the write promises (**after** the write lands — invalidating first would let a read in between recache the old records). Note this fixes a **pre-existing** hole too: `_allTournaments` already cached `tournamentAchievements`/`placements`, which could already show stale after an unlock-edit. **Deliberately out of scope:** the `matchCount` self-heal loop (rare repair path) and `openTournament`/`openMatch` (single-tournament fetches — each one cached widens the staleness surface). **Verified by differential test** against the pre-change file from git, driving all four views through a stubbed NewtonDB with **inverted read-resolution order** (later requests finish first, so any dependence on completion order rather than input order would surface): rendered output byte-identical in all four (1.6 KB–13 KB per view), DB reads **48 → 12** for 12 tournaments, peak concurrency 1 → 12, re-render after a toggle **48 → 0**, and `invalidateCache()` confirmed to really clear it. A mutant run (dashboard drops one match per tournament) is caught, and the harness fails loudly if a view captures no output — the first version of this test compared nothing for `renderDashboard` and silently passed. **BROWSER-VERIFIED 2026-09-06 (post-v5.1.6):** part 3 confirmed through the real `?tm` flow — tournament loaded from disk via Setup → Shared Tournaments, unlocked, a match edited, then both the Analytics **Leaderboard** and the register's **Matches** view showed the corrected figures *without a reload*. (Note for future testing: the entry point is Setup → Shared Tournaments under `?tm`, **not** the register's Bracket button — that button is gated on `NEWTON_APP_MODE === 'analytics'` and `?tm` forces full mode, so it is correctly absent there.) The harness stubs IndexedDB and the DOM, so it proves the aggregation logic, ordering and read counts, **not** real IndexedDB behavior or real rendering. |
...
| 6.8 ✅ DONE | Downstream undo-blocking logic duplicated | `js/bracket-rendering.js` | **DONE 2026-09-06, BROWSER-VERIFIED** (undo correctly blocked by both a live and a completed downstream match). (Plan's line numbers were stale; the two copies live in `isMatchUndoable()` and `getDetailedMatchState()`.) **Both were live** — `isMatchUndoable` drives the per-card undo affordance, `getDetailedMatchState` (via `getMatchProgressionText`) drives the hover status text and the Match Controls card. Textually they differed in three ways (early-`return false` vs. accumulating labels; `if`/`if` vs. `if`/`else if` on active-then-completed; `_undoableTable` vs. `_progressionTable`), but **semantically they were identical** — confirmed before touching anything by running both over all 49 combinations of winner-destination × loser-destination state (incl. the `active && completed` case that is the only input where the `if`/`else if` split could show). Extracted `getUndoBlockingMatches(matchId, history)` → `[{matchId, live}]` in winner-then-loser order, `[]` when nothing blocks (including no progression table / no entry). `isMatchUndoable` is now `return getUndoBlockingMatches(...).length === 0`; `getDetailedMatchState` maps the array to its `" (live)"` / `"X and Y"` text. Each caller **keeps** its own read-only, walkover and has-a-MANUAL/QR-transaction gates — those differ in return shape, and folding them in would make the helper a second undo-eligibility authority. `history` is a parameter so the render pass keeps reusing the 6.1 memo. Net −18 lines. **Verified by differential test** against the pre-change file from git: 1200 randomized tournament states (DE + SE × 8/16/32, incl. read-only, byes, walkovers, live matches, MANUAL/QR/AUTO), 32,400 assertions comparing `isMatchUndoable` + full `getDetailedMatchState` per match — **zero differences**; a mutant run (QR dropped from one downstream check, i.e. bug 1.2's exact shape) confirms the test discriminates. |
| 6.9 ✅ DONE | `exportTournament` duplicates `buildTournamentPayload` | `js/tournament-management.js` | **DONE 2026-08-09.** `exportTournament` now calls `buildTournamentPayload()` (−30 lines); added the missing `URL.revokeObjectURL` (blob URL was leaked); sanitized the filename **at source** in `buildTournamentPayload` (hostile chars → `-`, deterministic so re-uploads still overwrite, benefits both download + upload paths). Minor accepted behavior change: the corrupt-history `console.warn` on export is gone (proceeds with empty history, as uploads already did). |
| 6.10 ✅ DONE | Achievement-points formula ×4–5 | `js/newton-history.js` | **DONE 2026-08-09.** Extracted `_achPoints(stats, p)` (private, next to `_getActivePoints`); replaced all 5 byte-identical sites (leaderboard aggregation, `_computeAchievementPoints`, matches tab, match-detail points, and the match-detail table's `_playerPoints` closure). Pure extraction, numerically verified equivalent — zero behavior change. |
| 6.11 ✅ DONE (narrow extraction; the leaderboard split declined) | Players-tab aggregation ⊂ leaderboard aggregation | `js/newton-history.js` | **DONE 2026-09-06.** **Measured the overlap before acting, and the plan overstated it:** of `renderLeaderboard`'s ~110-line aggregation, only ~20 lines have a Players-tab counterpart — the win/loss scan (16 of 18 lines byte-identical; the differences are a comment and where the leaderboard continues into legs) and the player-key convention. Everything else (points, ranking/attendance layers, placement counts, achievement totals, personal bests, legs, three-dart averages) is leaderboard-only. So **splitting `renderLeaderboard` (292 lines) was declined** — the plan's own rule for oversized functions is "split when touched, not as a dedicated pass", and that split would be the largest refactor in Phase 6 spent to share ~20 lines. Its readability merit stands on its own and can ride along when that view is next worked on. **What was extracted instead — the part where divergence would actually reach a user:** (1) `_tallyMatchWinLoss(matches, playerMap)`, shared by both views, so the Players tab and the Leaderboard cannot report different W/L for the same player; (2) `_playerKey(name)` — **the bigger find: this normalization was repeated at 20 sites**, not 2. Analytics has no stable cross-tournament player id, so players are matched *by name*; every view that groups, counts or selects them must normalize identically or one person becomes two. Now defined once and used everywhere — zero raw `.trim().toLowerCase()` calls remain outside the helper. `_playerKey` returns `null` for a falsy name, exactly reproducing the `m.playerXName ? … : null` ternaries at the scan sites; at the selection sites the names are always non-empty strings, so the result is unchanged. **Verified** with the 6.5 differential harness (drives all four views, compares full rendered output) against the pre-change file, with the fixture strengthened for this item: the same people are spelled inconsistently across tournaments (leading/trailing whitespace, mixed case) and one match names a player absent from `tournamentAchievements`, so the normalization and the null-map-entry path are genuinely exercised. All four views byte-identical; a mutant that drops the `trim()` from `_playerKey` is caught in both the Dashboard and the Players tab. **Not covered by the harness:** the selection/toggle paths (`togglePlayer`, `_restorePlayerSelection`, header checkbox) — UI interactions, mechanical substitutions, argued equivalent above. Net +27 lines (the helpers carry the reasoning in JSDoc). |
| 6.12 ✅ DONE | Inline base64 visit decoding | `js/newton-history.js` | **DONE 2026-08-09.** Both inline `atob`/`Uint8Array` decodes (leaderboard averages + match-detail leg table) now call `NewtonStats.decodeVisits(leg.s, 0/1)`. Removed the now-unneeded try/catch at the detail site (decodeVisits never throws). Round-trip equivalence verified. |
| 6.13 ✅ DONE | `parseExcludedLanes` ≡ `parseExcludedLanesString` | `js/lane-management.js`, `js/results-config.js` | **DONE 2026-08-09.** Turned out not to be "two live copies" — `parseExcludedLanes` (lane-management.js) was byte-identical **and had zero callers repo-wide**. Deleted it as dead code; kept `parseExcludedLanesString` (results-config.js) which has the one caller. Pure 17-line deletion, CRLF preserved. |
| 6.14 ✅ DONE | `developerMode` localStorage IIFE ×3 | `js/results-config.js` (new helper), `js/tournament-management.js` (×2), `js/player-management.js` | **DONE 2026-08-09.** Extracted `isDeveloperMode()` in results-config.js; the three inline IIFEs now call it. Fixed the single-source violation — the helper reads the in-memory `config.ui.developerMode`, not a fresh `dartsConfig` localStorage parse. Equivalent in the render contexts where it's used (config is loaded + in sync). Net −17 lines. |
| 6.15 ✅ DONE | Paid-players sort + placement sort ×4 | `js/results-config.js` | **DONE 2026-08-09.** Was actually ×**3** identical (`updateResultsTable`, `generateResultsJSON`, `buildResultsCSVData`) — the `renderPlayerList`/player-management.js:645 sort the plan lumped in is a *different* comparator (name-only, no paid filter), left alone. Extracted `getSortedPaidPlayers()` (ranked-first by placement, then name); all 3 sites call it. Ordering verified identical. Net −16 lines. |
| 6.16 ✅ DONE | `resetZoom()` duplicates the per-format/size zoom-pan defaults from `renderBracket()` | `js/bracket-rendering.js` | **DONE 2026-08-09.** Values were currently *identical* between the two copies (verified by diff) — extracted `getDefaultView(format, bracketSize)` returning `{zoom,panX,panY}` (or `null` for DE non-standard sizes → caller leaves view unchanged, matching old behavior). Each caller keeps its own wrapper (renderBracket's `initialBracketRender` gate; resetZoom's `0.6/(0,0)` fallback). CRLF file — done via python. **Browser-verified 32P** (default zoom + Reset-zoom both correct); 8/16/SE share the identical mechanism with byte-identical values. |
| 6.17 ✅ DONE | `render32PlayerBacksideMatches` inlines `createBacksideBackground`'s 32P branch | `js/bracket-rendering.js` | **DONE 2026-08-09.** The shared function already had a byte-identical `case 32` branch; 32P now calls `createBacksideBackground(grid, 32, round1StartY, spacing)` like 8P/16P (compute round1StartY first, then call + append). Removed the inline div/dimension math. Net −21 lines, CRLF preserved (python). **Browser-verified 32P** (hard reload — backside gradient + labels correct). |
| 6.18 ✅ DONE | `updateMatchReferee` clear/assign paths ~45 duplicated lines | `js/bracket-rendering.js` | **DONE 2026-08-09.** Merged the clear + assign paths: each branch just sets `match.referee` and the transaction `description`; a shared tail builds the `ASSIGN_REFEREE` transaction (`afterState:{referee: parsedRefereeId}` — `null` for clear), saves, `renderBracket()`, refreshes Match Controls. Dropped both redundant `refreshAllRefereeDropdowns()` calls (renderBracket rebuilds bracket dropdowns; modal-refresh rebuilds CC dropdowns; function kept — still called from analytics.js + lane-management.js). Net −40 lines. **Browser-verified** (assign, clear, already-assigned-alert, and undo all working). |
| 6.19 ✅ DONE | 32P placement labels recompute match Y positions from scratch | `js/bracket-rendering.js`, `js/bracket-lines.js` | **DONE 2026-08-09.** `render32PlayerBacksideMatches` now captures each first-match Y **as rendered** (one-line `if (i===1) bsNY = matchY;` per round, no loop restructuring) into the `positions` object; `createBacksidePlacementLabels` case 32 reads `positions.bsNY` like 8P/16P — no more parallel recompute. Also dropped the confusing "round-3 uses BS-4-1 position" quirk: round 3 now reads BS-3-1's actual Y (which renders at the same Y as BS-4-1). **Per-round label Y proven numerically identical old-vs-new (all 7 rounds).** **Browser-verified 32P** (hard reload — labels identical to before). |
| 6.20 ✅ DONE | Chalker hardcoded `SHORT_LEG_THRESHOLDS` vs TM's configurable `config.legs.shortLegThreshold` | `js/qr-bridge.js`, `chalker/js/chalker.js`, + cache/blurb | **DONE 2026-08-09.** Turned out the `chalkerShortLegThreshold` Config setting **never reached the Chalker** — TM used it for awards, the Chalker used its own hardcoded table, so the badge could disagree with the award. Fix: TM now sends `slt` in the assignment QR payload; Chalker carries it into `config.shortLegThreshold` and a new `getShortLegThreshold(config)` helper prefers it (3 display sites), falling back to the scaling table for standalone matches. **Backward-compatible** (old QR w/o `slt` → table). Bumped Chalker cache (`v110`, `?v=13`); updated the Config blurb. **Needs Chalker device test** (assign a match with a non-21 threshold → badge uses it). |
| 6.21 ✅ CLOSED (won't sync) | `detectQRCode`/`isQRScanAvailable` duplicated TM↔Chalker, already drifted | `js/qr-bridge.js`, `chalker/js/chalker.js` | **CLOSED 2026-08-09 — the "keep in sync" advice was stale.** Diffed them: they've **legitimately diverged** — TM's `detectQRCode` has a jsQR fallback and `isQRScanAvailable` checks `BarcodeDetector \|\| jsQR`; the Chalker is BarcodeDetector-only with an **iOS image-capture** path (`isIOS()`). Syncing would break one platform. Instead of a "keep in sync" comment, added a **"these INTENTIONALLY differ, don't unify"** note to both. No code consolidation. |
