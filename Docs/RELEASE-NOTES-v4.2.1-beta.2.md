## NewTon DC Tournament Manager v4.2.1-beta.2 Release Notes
...
**NewTon DC Tournament Manager Version 4.2.1-beta.2** focuses on visual polish and discoverability. Match cards receive subtle refinements, eliminated players are now visually marked with strikethrough text, and a new optional landing page improves SEO for self-hosted instances.
...
- Optional landing page for Docker deployments (beta)

---

## Changes since beta.1

### Landing page moved into tournament.html

The landing page was previously a separate `index.php` file. This caused 404 errors on deployments using Nginx Proxy Manager (NPM), because NPM's advanced config would block the separate PHP file before it reached the container.

The landing page is now embedded at the top of `tournament.html` (which becomes `tournament.php` during Docker build). This eliminates the routing issue entirely — no extra files to whitelist, no separate nginx index directive needed.

- `index.php` has been removed
- The "Launch Tournament Manager" button now uses `?launch` to bypass the landing page
- nginx index directive updated to serve `tournament.php` directly
...
**What it includes:**
- App header (logo + title, no menu or clock)
- Feature descriptions
- Screenshots from the repository
- "Launch Tournament Manager" button
...
When `NEWTON_LANDING_PAGE` is not set (the default), visitors go straight to the tournament app as before. Adding `?launch` to the URL bypasses the landing page regardless of this setting.

This feature is marked as **beta** and may change in future releases.
...
The landing page is embedded in `tournament.html` (which becomes `tournament.php` in Docker). No separate `index.php` file is needed. The nginx index directive serves `tournament.php` as the default document.
...
None at time of release. The landing page is a beta feature — feedback welcome through the GitHub repository.

---

**NewTon DC Tournament Manager v4.2.1-beta.2** — cleaner cards, clearer brackets, better discoverability.
